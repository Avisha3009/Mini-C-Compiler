...
/**
    ...
    @return void None
*/
void InsertId(char *id,int lineno,int type,int offset){
    short k;
    char *temp;
    if( IsPresent(id) ){
        k = getType(id);
    ...
    if(Table[k].Identifier==NULL){
        ...
        */
        if(Table[k].next==NULL){
            Table[k].next = newTable(id,lineno,type,offset);
        }
        SymbolTable *temp = Table[k].next;
        while(temp->next!=NULL)temp = temp->next;
        temp->next = newTable(id,lineno,type,offset);
    }
}
...
void printStable(){
	printf("Identifier\tType\tWidth\tOffset\n");
	for(size_t i=0;i<MOD;i++){
       SymbolTable *temp;
       if(Table[i].Identifier!=NULL){
            char* typestr = "\0";
            printf("%s\t\t",Table[i].Identifier);
            printType(Table[i].attr->type);
            printf("%d ",Table[i].attr->offset);
            printf("\n");
            //printf("%s ",typestr);
            ...
            while(temp!=NULL){
               char* typestr = "\0";
            	...
            	printType(temp->attr->type);
            	printf("%d ",temp->attr->offset);
            	printf("\n");
               temp=temp->next;
            }
